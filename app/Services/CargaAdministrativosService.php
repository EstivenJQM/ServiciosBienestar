...
class CargaAdministrativosService
{
    ...
    private const NOMBRE_ROL = 'Administrativo';
    ...
    public function cargar(UploadedFile $archivo, int $idPeriodo): array
    {
        ...
        $this->idTipoEmpleado = (int) DB::table('tipo_empleado')->where('nombre', 'Administrativo')->value('id_tipo_empleado');

        if (! $this->idRol || ! $this->idTipoEmpleado) {
            return [
                'creados'      => 0,
                'actualizados' => 0,
                'errores'      => ['No se encontró el rol "Empleado" o el tipo "Administrativo" en el sistema.'],
                'total'        => 0,
            ];
        }
