...
class CargaDocentesService
{
    ...
    private const NOMBRE_ROL = 'Docente';
    ...
    public function cargar(UploadedFile $archivo, int $idPeriodo): array
    {
        ...
        $this->idTipoEmpleado = (int) DB::table('tipo_empleado')->where('nombre', 'Docente')->value('id_tipo_empleado');

        if (! $this->idRol || ! $this->idTipoEmpleado) {
            return [
                'creados'      => 0,
                'actualizados' => 0,
                'errores'      => ['No se encontró el rol "Empleado" o el tipo "Docente" en el sistema.'],
                'total'        => 0,
            ];
        }
    ...
    private function guardarInconsistencia(
        ...
        try {
            ...
            Log::error('No se pudo guardar inconsistencia docente: ' . $ex->getMessage());
        }
    }
